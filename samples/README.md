...
Runnable Java samples that exercise the v3.2.2 Trade APIs. For an overview of the SDK itself, see the [top-level README](../README.md).
...
- **Maven 3.6+**.
- The SDK JAR at `../dist/samco-bridge-java-3.2.2.jar`.
...
The samples `pom.xml` references the SDK JAR directly via a `system`-scoped dependency on `../dist/samco-bridge-java-3.2.2.jar`, so no separate `mvn install:install-file` bootstrap is required — a fresh clone builds out of the box.

The build produces a self-contained fat-jar at
`target/samco-bridge-java-samples-3.2.2.jar` and seeds an editable
...
```bash
java -jar target/samco-bridge-java-samples-3.2.2.jar
```
